...
def get_contacts_menu():
    ...
    builder.button(text="🌐 Перейти на сайт", url=URL)
...
def get_menu_about_us():
    ...
    builder.button(text="🌐 Перейти на сайт", url=f"{URL}/podderzhat-gazetu/")
    builder.button(text="🔙 Главное меню", callback_data="back_to_main")
    builder.adjust(1)
    return builder.as_markup()
...
def get_menu_newspaper():
    ...
    builder = InlineKeyboardBuilder()
    builder.button(text="🌐 Перейти на сайт", url=f"{URL}/arhivy-gazety/")
    builder.button(text='🔙 Главное меню', callback_data='back_to_main')
    builder.adjust(2)
    return builder.as_markup()

def get_menu_newspaper_search(newspapers):
    ''' Клавиатура меню "Поиск по архиву" Telegram-бота «Доброе Слово». '''
    builder = InlineKeyboardBuilder()
    for newspaper in newspapers:
        # Форматируем текст кнопки красивее
        title_clean = newspaper['title'].replace('Газета ', '').replace('-', ' ')
        builder.button(
            text=f"📰 {title_clean}",
            callback_data=f"newspaper_{newspaper['year']}_{newspaper['issue']}"
        )
    builder.button(text="🌐 Перейти на сайт", url=f"{URL}/arhivy-gazety/")
    builder.button(text='🔙 Главное меню', callback_data='back_to_main')
    builder.adjust(2)
